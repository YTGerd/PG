...
class Result():
    def __init__(self):
        self.RSSI = self.DEV = self.dist = self.RSSI_kal = 0
                ...
                if packet_class == 0x06: # gap
                    if packet_command == 0x00: # scan_response
                        ...
                        if display:
                            ...
                            count=0
                            for i in disp_list:
                                if str(i).find("8856")!=-1:
                                    count+=1
                            if count:
                                self.RSSI = disp_list[1]
                                self.kalman_filter.input_latest_noisy_measurement(self.RSSI)
                                self.RSSI_kal = self.kalman_filter.get_latest_estimated_measurement()
                                print self.RSSI,self.RSSI_kal
